...
Use this guide for automated work in the `surgeist-retained` crate repo.
...
This repo owns the `surgeist-retained` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-retained` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-retained` owns retained identity, retained state, tree identity, and stable handles.
...
1. Confirm the work belongs in `surgeist-retained`.
